...
import { getProducts,setFilteredProducts,addToCart  } from '../Redux/productSlice';
...
const ProductList = () => {
    ...
    const { productitem, loading, filteredProductItems,error } = useSelector((state) => state.products);
    ...
    const handleAddToOrder = (product) => {
        ...
        // Dispatch to Redux store to add product to the cart
    ...
    if (!productitem || productitem.length === 0) return <p>No products available</p>;
    return (
        <>
                        ...
                        filteredProductItems.map((product) => (
                            <div className='col-12 col-md-6 col-sm-12 col-lg-3 mb-3' key={product.id}>
                                ...
                                <div className="d-flex justify-content-between mt-2 gap-4">
                                    <p className="mb-0 product-title">{product.title}</p>
                ...
                {isOrderView ? (
                    <ul>
                        {order.map((item, idx) => (
                            <div key={idx} className="d-flex align-items-center justify-content-between mb-3 border-bottom pb-2">
                                <img src={item.images?.[0]} alt={item.title} style={{ width: '60px', height: '60px', objectFit: 'cover' }} className="rounded product-images" />
                    ...
                    <div>
                        {selectproduct?.images?.[0] && (
                            <img src={selectproduct.images[0]} alt={selectproduct.title} className='img-fluid product-images mb-3' />
                        )}
